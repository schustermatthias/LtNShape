            ...
            for (rTdx = 0; rTdx < Rdx; rTdx++) {
                for (i = 0; i < quadRule.nPy; i++) {
                    ...
                    if (conf.is_singularKernel){
                        ...
                        if ((aT.label == 1) or (bT.label == 1)) {
                            for (a=0; a < mesh.dVertex; a++) {
                                double value_2 = (adjoint_values[0] - adjoint_values[1])
                                                 * (state_values[0] - state_values[1])*quadRule.dy[i] *quadRule.dx[k]*rTdet*aT.absDet;
                                termNonloc[2*a] -= 0.5*(8 / (M_PI * pow(mesh.sqdelta, 3))) * value_2
                                                 * (x[0] - physical_quad[0]) * quadRule.psix(a, k);
                                termNonloc[2*a + 1] -= 0.5*(8 / (M_PI * pow(mesh.sqdelta, 3))) * value_2
                                                    * (x[1] - physical_quad[1]) * quadRule.psix(a, k);
                                termNonlocPrime[2*a] += 0.5*(8 / (M_PI * pow(mesh.sqdelta, 3))) * value_2
                                                     * (x[0] - physical_quad[0]) * psi_value[a];
                                termNonlocPrime[2*a + 1] += 0.5*(8 / (M_PI * pow(mesh.sqdelta, 3))) * value_2
                                                       * (x[1] - physical_quad[1]) * psi_value[a];
                            }
                        }
                    }
                }
