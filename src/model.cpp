...
void kernel_constant(const double *x, const long labelx, const double *y, const long labely, const MeshType &mesh,
    ...
    *kernel_val = 4 / (M_PI * pow(mesh.sqdelta, 2)) * (1 - (dist/mesh.sqdelta) );
}
...
void kernel_constant_neumann_2(const double *x, const long labelx, const double *y, const long labely,
                               const MeshType &mesh, double *kernel_val) {
    double dist;
    dist = vec_sqL2dist(x, y, 2);
    if (labelx == 1 or labely == 1) {
        *kernel_val = (4 / (M_PI * pow(mesh.sqdelta, 2))) * (1 - 0.5 * (dist/mesh.sqdelta) );
    } else {
        *kernel_val = 0.0;
    }
}
