...
void lookup_configuration(ConfigurationType & conf, int verbose=0){
    ...
    map<string, void (*)(const double * x, long labelx, const double * y, long labely,
            ...
            {"constant_neumann_2", kernel_constant_neumann_2},
                                            ...
                                            if (aT.label == 1){
                                                {
                                                    if (mesh.LabelVerts[aAdx[a]] > 0) {
                                                        {
                                                            Ad[aAdx[a]] += termLocal[0] * div_a[2*a] + termNonloc[2*a];
                                                            Ad[aAdx[a] + mesh.K] += termLocal[0] * div_a[2*a + 1]
                                                                                    + termNonloc[2*a + 1];
                                                        }
                                                    }
                                                    if (mesh.LabelVerts[bAdx[a]] > 0) {
                                                        {
                                                            Ad[bAdx[a]] += termLocal[0] * div_b[2*a] + termNonlocPrime[2*a];
                                                            Ad[bAdx[a] + mesh.K] += termLocal[0] * div_b[2*a + 1]
                                                                                    + termNonlocPrime[2*a + 1];
                                                        }
                                                    }
                                                }
                                            }
